...
class ModelView: NSObject{
    ...
    convenience init(controlInput: UIControl, labelInput: UILabel) {
        ...
        self.colorButton = self.control.backgroundColor!

        let clouser: ClouserRun = { [unowned self] controlSelf in
            if(self.count != 0){
               let number = Int(arc4random()%1000)
                self.label.text = "\(number)"
                let color = (CGFloat)(number)/1000
                self.control.backgroundColor = UIColor.init(displayP3Red: color, green: color, blue: color, alpha: 1)
            }
            self.count = self.count + 1
        }

        _ = self.control.avControl.taap.asObservable()
